...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    if (mobile) {
      // On mobile: show everything immediately, no GSAP
    ...
    if (title) {
      ...
      gsap.fromTo(
        ...
        { y: 0, opacity: 1, rotateX: 0, duration: 1, stagger: 0.04, delay: 0.2, ease: "power3.out" }
      );
    }
    ...
    gsap.fromTo(".hero-subtitle", { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, delay: 1.8, ease: "power3.out" });
    gsap.fromTo(".hero-cta", { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, stagger: 0.15, delay: 2.2, ease: "power3.out" });
    gsap.fromTo(".hero-scroll-indicator", { opacity: 0 }, { opacity: 1, duration: 0.6, delay: 3, ease: "power2.out" });
    ...
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto w-full grid md:grid-cols-12 gap-8 items-center">
        <div className="md:col-span-8">
          ...
          >
            {isMobile ? (
              <>Somos <span className="text-gradient">Kairox.</span></>
            ) : (
                  ...
                  <span
                    className={`hero-char inline-block ${
                      char === "K" || char === "a" || char === "i" || char === "r" || char === "o" || char === "x" || char === "."
                        ? i >= titleText.indexOf("K", 6) ? "text-gradient" : ""
                        : ""
                    }`}
                  >
                    {char === " " ? "\u00A0" : char}
          ...
          <p className={`hero-subtitle text-lg sm:text-xl text-white/50 max-w-lg mb-10 leading-relaxed ${isMobile ? "" : "opacity-0"}`}>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <a
              href="#contact"
              className={`hero-cta px-8 py-4 rounded-full bg-cyan text-[#0a0a12] font-semibold text-sm hover:bg-cyan/90 transition-all duration-200 glow-cyan inline-block text-center ${isMobile ? "" : "opacity-0"}`}
            >
              Hablemos
            </a>
            <a
              href="#services"
              className={`hero-cta px-8 py-4 rounded-full border border-white/10 text-white/70 font-medium text-sm hover:border-white/25 hover:text-white transition-all duration-200 inline-block text-center ${isMobile ? "" : "opacity-0"}`}
            >
